//! How this destination rolls its output into part files: the vocabulary the Iceberg destination's
//! own config document speaks.
//!
//! Owned here rather than shared, because it IS this connector's
//! vocabulary — the table's own writer rolls the files, so a size bound this
//! connector cannot impose is refused at its gate rather than
//! silently ignored. A neighbouring
