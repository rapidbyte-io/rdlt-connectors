#!/usr/bin/env python3
"""Bootstrap the example's Polaris catalog: wait for the API, create
the warehouse bucket on RUSTFS, create the catalog (dual endpoints:
CLIENTS on the host reach RUSTFS at 127.0.0.1, POLARIS reaches it over
the compose network), grant the admin role. Stdlib-only. Adapted from
rdlt's own test fixture; runs as the one-shot `bootstrap` compose
service.

Usage: bootstrap.py <polaris-api> <rustfs-client-endpoint>
         <rustfs-polaris-endpoint> <s3-access> <s3-secret>
         <client-id> <client-secret> <warehouse> <bucket>
         <rustfs-bootstrap-endpoint>
"""
import datetime
import hashlib
import hmac
...
def sigv4_put_bucket():
    # The bucket is created over the endpoint THIS process can reach
    # (the compose network); the catalog below stores the endpoint the
    # HOST's rdlt will be vended.
    host = s3_bootstrap_ep.split("://", 1)[1]
        ...
        body={
            "catalog": {
                ...
                "storageConfigInfo": {
                    ...
                    # store (the compose service name) for metadata writes.
